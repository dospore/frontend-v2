    ...
    >
      ...
      <template v-slot:detailsCell="action">
        <div class="px-6 py-4 flex -mt-1 flex-wrap">
          ...
          <span class="font-numeric">{{ action.tokenInAmount }}</span>
          <BalAsset :address="action.tokenOut" class="mr-2 flex-shrink-0" />
          <span class="font-numeric">{{ action.tokenOutAmount }}</span>
        </div>
      </template>
...
export default {
  ...
  props: {
    ...
    poolSwaps: {
      type: Array as PropType<PoolSwap[]>,
      required: true
    },
    isLoading: {
      type: Boolean,
      ...
      props.isLoading
        ...
        : props.poolSwaps.map(
            ({
              ...
              return {
                value: fNum(priceFor(tokenIn), 'usd'),
                tokenIn,
                tokenOut,
                tokenInAmount,
                tokenOutAmount,
